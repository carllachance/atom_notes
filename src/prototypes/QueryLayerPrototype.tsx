...
import { queryPrototypeItems, queryPrototypePrompts, type QueryPrototypeItem } from './queryLayerPrototype.mock';
...
function rankItem(item: QueryPrototypeItem, query: string, category: QueryCategory) {
  ...
  return item.score + queryBoost + freshnessBoost + typeBoost;
}
...
export function QueryLayerPrototype() {
  ...
  const rankedItems = useMemo(() => {
    return queryPrototypeItems
      .map((item) => ({ item, rank: rankItem(item, query, category) }))
            ...
            {rankedItems.length ? (
              rankedItems.map(({ item, rank }, index) => (
                <button
                  key={item.id}
                  type="button"
                  className={selectedItem?.id === item.id ? 'query-prototype__result query-prototype__result--active' : 'query-prototype__result'}
